      ...
      <div className="w-full">
        ...
        <div className="relative">
          <select
            id={selectId}
            className={cn(
              'flex h-12 w-full appearance-none rounded-lg border border-slate-600 bg-[#1a1f2e] px-4 py-2 pr-10 text-base text-slate-100',
          ...
          >
            {options.map((option) => (
              <option key={option.value} value={option.value} className="bg-[#1a1f2e] text-slate-100">
                {option.label}
              </option>
            ))}
          </select>
