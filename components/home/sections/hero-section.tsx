...
import { Loader2, ArrowRight, Check, Clock, Shield } from 'lucide-react'
...
export function HeroSection({ onGetStarted, isCreating, error }: HeroSectionProps) {
  ...
  return (
    <section
      id="hero"
      aria-label="Hero"
      className="relative overflow-hidden min-h-[85svh] lg:min-h-[90svh] flex items-center"
    >
      <div className="absolute inset-0">
        <HeroSlider />
        {/* Slightly darker overlay so copy reads clean over photos */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#0c0f14]/85 via-[#0c0f14]/55 to-[#0c0f14]/92" />
      ...
      <div className="relative mx-auto w-full max-w-6xl px-4 py-16 md:py-24">
        <div className="max-w-4xl">
          ...
          <p className="mt-6 max-w-2xl text-lg md:text-xl text-slate-300 leading-relaxed animate-hero-subtitle delay-100">
            A free, honest estimate in two minutes &mdash; built from real local pricing.
            Plus everything you need to navigate insurance, financing, and assistance.
          </p>

          <div className="mt-9 flex flex-col sm:flex-row sm:items-center gap-4 animate-hero-cta delay-200">
            <Button
              variant="primary"
              size="xl"
          ...
          <ul className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-x-8 gap-y-4 max-w-2xl animate-hero-subtitle delay-300">
            ...
            <li className="flex items-center gap-2.5 text-sm text-slate-300">
              ...
              <span>Insurance + financing guidance</span>
            </li>
            <li className="flex items-center gap-2.5 text-sm text-slate-300">
          ...
          {ownerName && (
            <p className="mt-8 text-sm text-slate-400 animate-hero-subtitle delay-400">
              A pricing tool from{' '}
