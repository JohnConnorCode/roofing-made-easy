  ...
  if (error || !customer) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-4 text-slate-400">{error || 'Customer not found'}</p>
    ...
    <AdminPageTransition className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-slate-400">Customer since {formatDate(customer.created_at)}</p>
        </div>
              ...
              {isEditing ? (
                ...
                <dl className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <dd className="text-slate-50">{name}</dd>
                  </div>
                  <div>
                    <dt className="text-sm text-slate-400">Email</dt>
                    <dd className="text-slate-50 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <dd className="text-slate-50 capitalize">{customer.role}</dd>
                  </div>
                </dl>
              )}
            </CardContent>
          ...
          <Card className="border-white/5 bg-slate-950/40 backdrop-blur-xl">
            ...
            <CardContent>
              {customer.leads.length === 0 ? (
                <p className="text-slate-400 text-center py-4">No leads linked to this customer</p>
              ) : (
                  ...
                  {customer.leads.map((lead) => {
                    ...
                    return (
                      <Link
                        key={lead.id}
                        href={`/leads/${lead.id}`}
                        className="block p-4 bg-slate-900/40 rounded-lg hover:bg-slate-900/60 transition-colors"
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <div className="text-right">
                            <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${STATUS_STYLES[lead.status] || 'bg-slate-900/60'}`}>
          ...
          <Card className="border-white/5 bg-slate-950/40 backdrop-blur-xl">
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-slate-50">{customer.leads.length}</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <Card className="border-white/5 bg-slate-950/40 backdrop-blur-xl">
            ...
            <CardContent>
              {customer.financing_applications.length === 0 ? (
                <p className="text-sm text-slate-400">No financing applications</p>
              ) : (
                <div className="space-y-2">
                  {customer.financing_applications.map((app) => (
                    <div key={app.id} className="p-3 bg-slate-900/40 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-slate-50">
                          {formatCurrency(app.amount_requested)}
                        </span>
                        <span className={`px-2 py-0.5 text-xs rounded-full ${FINANCING_STATUS_STYLES[app.status] || 'bg-slate-900/60'}`}>
          ...
          <Card className="border-white/5 bg-slate-950/40 backdrop-blur-xl">
            ...
            <CardContent>
              {customer.insurance_claims.length === 0 ? (
                <p className="text-sm text-slate-400">No insurance claims</p>
              ) : (
                <div className="space-y-2">
                  {customer.insurance_claims.map((claim) => (
                    <div key={claim.id} className="p-3 bg-slate-900/40 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-slate-50 text-sm">
                          {claim.insurance_company}
                        </span>
                        <span className={`px-2 py-0.5 text-xs rounded-full ${INSURANCE_STATUS_STYLES[claim.status] || 'bg-slate-900/60'}`}>
